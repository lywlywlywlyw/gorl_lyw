...
def forward_fm_batch(
    ...
    return decoder.sample_action_from_z(
        observations, latents, jax.random.PRNGKey(0), deterministic=True
    )
...
def save_compatible_checkpoint(
    ...
    # train_encoder_ppo.py interprets it as Decoder1StepFMConfig. One file cannot
    # place both types under the same key. The pipeline consumes this final
    # checkpoint as a decoder, so "config" stays Decoder1StepFMConfig. A separate
...
def main(config: FrozenOfflineConfig) -> None:
    ...
    try:
        ...
        print(f"Frozen decoder validation MeanFlow loss: {best_validation:.6f}")
        ...
        frozen_record = {
            ...
            "global_step": global_step,
            "decoder/best_validation_meanflow_loss": best_validation,
            **frozen_metrics,
        }
