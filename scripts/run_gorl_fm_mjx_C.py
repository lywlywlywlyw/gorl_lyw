...
class Config:
    ...
    fm_num_layers: int = 4
    fm_value_bins: int = 10
    fm_bin_keep_ratio: float = 0.5
    fm_uniform_ratio: float = 0.2
...
def validate_config(config: Config) -> None:
    ...
    if config.fm_value_bins < 2:
        raise ValueError("fm_value_bins must be at least 2.")
    if not 0 < config.fm_bin_keep_ratio <= 1:
        raise ValueError("fm_bin_keep_ratio must be in (0, 1].")
    if not 0 <= config.fm_uniform_ratio < 1:
        raise ValueError("fm_uniform_ratio must be in [0, 1).")
...
def select_decoder_data(
    ...
    advantages: np.ndarray, state_values: np.ndarray,
    config: Config, rng: np.random.Generator,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    n = min(len(obs), config.fm_max_samples)
    nu = int(round(n * config.fm_uniform_ratio)); ns = n - nu
    edges = np.quantile(state_values, np.linspace(0, 1, config.fm_value_bins + 1))
    ids = np.searchsorted(edges[1:-1], state_values, side="right")
    counts = np.full(config.fm_value_bins, ns // config.fm_value_bins)
    counts[:ns % config.fm_value_bins] += 1
    selected = []
    for bin_id, count in enumerate(counts):
        members = np.flatnonzero(ids == bin_id)
        if not len(members): members = np.arange(len(obs))
        cutoff = np.quantile(advantages[members], 1 - config.fm_bin_keep_ratio)
        high = members[advantages[members] >= cutoff]
        selected.append(rng.choice(high, count, replace=len(high) < count))
    rows = np.concatenate(selected + [rng.choice(
        len(obs), nu, replace=len(obs) < nu
    )])
    rng.shuffle(rows)
    return obs[rows], actions[rows], rewards[rows]
...
def train_decoder(
    ...
    obs, actions, rewards = select_decoder_data(
        obs, actions, rewards, advantages, state_values, config, rng
    )
...
def main(config: Config) -> None:
    ...
    try:
        for stage, stage_steps in enumerate(timesteps):
            ...
            agent = train_decoder(
                agent, obs, actions, rewards, advantages, state_values, config, stage, metrics_path,
