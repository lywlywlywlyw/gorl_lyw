...
class Config:
    ...
    fm_num_layers: int = 4
    fm_high_quality_ratio: float = 0.7
...
def validate_config(config: Config) -> None:
    ...
    if not 0 <= config.fm_high_quality_ratio <= 1:
        raise ValueError("fm_high_quality_ratio must be in [0, 1].")
    if not 0 < config.fm_high_quality_percentile <= 1:
...
def select_decoder_data(
    ...
    advantages: np.ndarray, config: Config, rng: np.random.Generator,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    n = min(len(obs), config.fm_max_samples)
    high = np.flatnonzero(advantages >= np.quantile(
        advantages, config.fm_high_quality_percentile
    ))
    nh = int(round(n * config.fm_high_quality_ratio))
    rows = np.concatenate((
        rng.choice(high, nh, replace=len(high) < nh),
        rng.choice(len(obs), n - nh, replace=len(obs) < n - nh),
    ))
    rng.shuffle(rows)
...
def train_decoder(
    ...
    obs, actions, rewards = select_decoder_data(
        obs, actions, rewards, advantages, config, rng
    )
