...
# Support the documented invocation ``python scripts/run_gorl_fm.py`` without
...
def main(
    ...
    Args:
        ...
        seed: Random seed
        use_offline_checkpoint: Start stage 0 from an offline-trained FM decoder.
        offline_checkpoint_path: Offline ``checkpoint_final.pkl`` or periodic
            ``checkpoint_step_*.pkl`` generated by
            ``run_offline_fm_frozen_robomimic.py``.
    """
    config = TrainingConfig().to_dict() | EnvConfig().to_dict()
    if use_offline_checkpoint and offline_checkpoint_path is None:
        raise ValueError(
            "--offline-checkpoint-path is required when "
            "--use-offline-checkpoint is enabled."
        )
    if not use_offline_checkpoint and offline_checkpoint_path is not None:
        raise ValueError(
            "--offline-checkpoint-path was provided but --use-offline-checkpoint "
            "is disabled. Enable it or remove the path."
        )
    # Create unique run identifier
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    run_id = f"gorl_fm_{config['env_name']}_seed{config['seed']}_{timestamp}"
    ...
    if use_offline_checkpoint:
        ...
        print(f"Initial offline FM checkpoint: {initial_offline_checkpoint}")
    ...
    for stage in range(config['num_stages']):
        ...
        # Adaptive parameters
        if stage == 0:
            stage_max_grad_norm = config['max_grad_norm']
            stage_clipping_epsilon = 0.15
            if config['z_regularization'] is not None:
                stage_z_regularization = config['z_regularization']
            else:
                stage_z_regularization = 0.0 if config['env_name'] == "BallInCup" else 0.0005
        else:
        ...
        cmd = " ".join([
            "MUJOCO_GL=egl PYOPENGL_PLATFORM=egl python scripts/components/train_encoder_ppo.py",
        ...
        fm_cmd_parts = [
            ...
            f"--data_path {shlex.quote(str(data_file))}",
            f"--output_dir {shlex.quote(str(stage_dir))}",
            f"--stage {stage}",
    ...
    # Pipeline Complete
    # =========================================================================
    print(f"\nGoRL (FM) complete - {config['num_stages']} stages")
