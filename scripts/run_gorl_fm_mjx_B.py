...
class Config:
    ...
    fm_num_layers: int = 4
    fm_advantage_beta: float = 1.0
    fm_weight_epsilon: float = 1e-8
    fm_weight_min: float = 0.1
    fm_weight_max: float = 5.0
    ...
    def rollout_with_actions(
        ...
        def step(carry: MjxRolloutState, _):
            ...
            with jdc.copy_and_mutate(carry) as updated:
                ...
                updated.prng = next_key
            transition = (
                carry.env_state.obs, next_state.obs, action, next_state.reward,
                (next_steps >= episode_length).astype(jnp.float32),
                1.0 - next_state.done.astype(jnp.float32),
            )
            return updated, transition
...
def validate_config(config: Config) -> None:
    ...
    if config.fm_advantage_beta <= 0:
        raise ValueError("fm_advantage_beta must be positive.")
    if not 0 < config.fm_weight_min <= config.fm_weight_max:
        raise ValueError("FM weight bounds must satisfy 0 < min <= max.")
...
def collect_data(
    agent: EncoderFMAgent, env: Any, config: Config, stage: int,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    rollout = MjxRolloutState.init(
        env, prng=jax.random.key(config.seed + stage + 50_000),
        num_envs=config.num_envs,
    )
    states, actions, rewards, advantages, state_values = [], [], [], [], []
    for _ in trange(config.data_collection_iterations, desc="Collect decoder data"):
        rollout, transition = rollout.rollout_with_actions(
            agent, episode_length=config.episode_length,
            iterations_per_env=config.collection_steps_per_iteration,
            apply_tanh_in_rollout=config.apply_tanh_in_rollout,
        )
        obs, next_obs, act, rew, truncation, discount = transition
        mean, std = agent.ppo_z_state.obs_stats.mean, agent.ppo_z_state.obs_stats.std
        values = networks.value_mlp_fwd(
            agent.ppo_z_state.params.value, (obs - mean) / std
        )
        bootstrap = networks.value_mlp_fwd(
            agent.ppo_z_state.params.value, (next_obs[-1:] - mean) / std
        )
        _, advantage = rollouts.compute_gae(
            truncation=truncation, discount=discount * config.discounting,
            rewards=rew * config.reward_scaling, values=values,
            bootstrap_value=bootstrap, gae_lambda=config.gae_lambda,
        )
        ...
        advantages.append(np.asarray(advantage).reshape(-1))
        state_values.append(np.asarray(values).reshape(-1))
...
def train_decoder(
    ...
    if len(obs) > config.fm_max_samples:
        ...
        obs, actions, rewards, advantages = (
            obs[rows], actions[rows], rewards[rows], advantages[rows]
        )
    standardized = (advantages - advantages.mean()) / (advantages.std() + config.fm_weight_epsilon)
    weights = config.fm_weight_epsilon + np.clip(np.exp(standardized / config.fm_advantage_beta),
                      config.fm_weight_min, config.fm_weight_max).astype(np.float32)
    ...
    for epoch in trange(config.fm_num_epochs, desc=f"Stage {stage} FM"):
        ...
        for start in range(0, len(shuffled) - config.fm_batch_size + 1,
            ...
            batch_obs, batch_actions = jnp.asarray(obs[rows]), jnp.asarray(actions[rows])
            batch_weights = jnp.asarray(weights[rows])
            obs_norm = ((batch_obs - decoder.obs_stats.mean)
                        / (decoder.obs_stats.std + 1e-8))
            eps_key, time_key, next_key = jax.random.split(decoder.prng, 3)
            eps = jax.random.normal(eps_key, (
                len(rows), decoder.config.n_samples_per_action, batch_actions.shape[-1]
            ))
            times = jax.random.uniform(time_key, (
                len(rows), decoder.config.n_samples_per_action, 1
            ))
            def loss_fn(params):
                state = jdc.replace(decoder, params=params)
                losses_per_row = jnp.mean(state.compute_cfm_loss(
                    obs_norm, batch_actions, eps, times
                ), axis=-1)
                return jnp.sum(batch_weights * losses_per_row) / jnp.sum(batch_weights)
            loss, grads = jax.value_and_grad(loss_fn)(decoder.params)
            updates, opt_state = decoder.opt.update(grads, decoder.opt_state)
            with jdc.copy_and_mutate(decoder) as decoder:
                decoder.params = optax.apply_updates(decoder.params, updates)
                decoder.opt_state = opt_state
                decoder.prng = next_key
                decoder.steps = decoder.steps + 1
...
def main(config: Config) -> None:
    ...
    try:
        for stage, stage_steps in enumerate(timesteps):
            ...
            obs, actions, rewards, advantages, state_values = collect_data(
                agent, env, config, stage
            )
            ...
            agent = train_decoder(
                agent, obs, actions, rewards, advantages, config, stage, metrics_path,
